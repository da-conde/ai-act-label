...
def _get_marie_columns(df_plan: pd.DataFrame) -> List[str]:
    """Alle Spalten vom Typ Marie__<Kategorie>."""
...
def _compute_progress(df_plan: pd.DataFrame, marie_cols: List[str], skipped_ids: List[str]) -> Dict:
    ...
    - Eine README gilt als "done", wenn mindestens eine Marie__-Spalte (0 oder 1) gesetzt ist
      ODER wenn sie in skipped_marie.csv steht.
    """
    ...
    for _, row in df_plan.iterrows():
        ...
        # Fertig, wenn mindestens eine Marie-Spalte gesetzt ist (0 oder 1)
        any_label = False
...
def render():
    ...
    # ------------------------------------------------
    # 2) Marie-Spalten & Kategorien aus dem Plan bestimmen
    # ------------------------------------------------
    marie_cols = _get_marie_columns(df_plan)
    if not marie_cols:
        st.error(
            "Im Labeling-Plan wurden keine Spalten vom Typ `Marie__<Kategorie>` gefunden. "
            "Bitte prüfe die Spaltennamen in `label.csv`."
        )
        return

    # Mapping: Kategorie-Name <-> Marie-Spalte
    ...
    # ------------------------------------------------
    # 7) Labeling-UI – direkt auf Basis der Session-Kopie (Marie-Spalten)
    # ------------------------------------------------
    ...
    with col1:
        if st.button("💾 Label lokal speichern & nächste README", key="ml_save_next"):
            ...
            if not mask_doc.any():
                ...
                # Nur Marie__-Spalten in der Session-Kopie aktualisieren
