# tabs/tab_labeling_marie.py  (ONLINE / Streamlit Cloud Variante – Marie)
...
def _reload_categories():
    ...
    st.session_state["marie_categories_cache"] = _load_categories_raw()
...
# --------------------------------------------------------------------
# YAML-Frontmatter entfernen & Sanitizer (ONLINE: wie Daniel v2 – mit html.escape)
# --------------------------------------------------------------------
...
def _highlight_keywords_multi(text: str, kw_color_pairs: List[Dict[str, str]]) -> str:
    ...
    for pair in sorted_pairs:
        ...
        if not kw:
            continue
        # Keywords müssen zum escaped Text passen
...
def _append_skipped_id(corpus_doc_id: str, filename: str):
    """doc_id + filename in skipped_marie.csv ergänzen (Drive + Cache)."""
    try:
        labelplan_folder_id = _get_labelplan_folder_id()
    except Exception as e:
...
# --------------------------------------------------------------------
# Fortschritt – direkt aus label.csv (Marie-Spalten)
# --------------------------------------------------------------------

def _get_marie_columns(df_plan: pd.DataFrame) -> List[str]:
    return [c for c in df_plan.columns if c.startswith("Marie__")]


def _compute_progress(df_plan: pd.DataFrame, marie_cols: List[str], skipped_ids: List[str]) -> Dict:
    """Done wenn mind. eine Marie__-Spalte gesetzt (0/1/2/...) oder geskippt."""
...
def render():
    st.subheader("🧩 Labeling – Marie (Online)")

    # Session-State-Init (global – Daniel & Marie teilen sich Versionen)
    ...
    with st.expander("⚙️ Optionen", expanded=False):
        ...
        with col_opt3:
            if st.button("🔄 Labelplan von Drive neu laden", key="reload_labelplan_btn_marie"):
                ...
                st.session_state["marie_labelplan_last_loaded"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    # 2) Marie-Spalten
    marie_cols = _get_marie_columns(df_plan)
    if not marie_cols:
        st.error("Im Labeling-Plan wurden keine Spalten vom Typ `Marie__<Kategorie>` gefunden.")
        return

    cat_to_col: Dict[str, str] = {}
    for col in marie_cols:
        cat_name = col.split("Marie__", 1)[1].lstrip("_")
    ...
    prog = _compute_progress(df_plan, marie_cols, skipped)
    ...
    manual_idx = st.session_state.get("ml_manual_doc_index", None)
        ...
        for cat, col in zip(row_cats, cols):
            with col:
                ...
                label_widgets[cat] = st.segmented_control(
                    ...
                    key=f"{doc_id}__{cat}__marie_v2",
                )
    ...
    with col1:
        if st.button("💾 Label lokal speichern & nächste README", key="ml_save_next"):
    ...
    # 9) Sync to Drive (nur Marie-Spalten)
    st.markdown("---")
    st.markdown("### Synchronisation mit Google Drive")

    col_sync1, col_sync2 = st.columns(2)
    with col_sync1:
        dirty = st.session_state.get("df_dirty_marie", False)
        status_text = "🟡 nicht hochgeladene Änderungen" if dirty else "🟢 alle Änderungen auf Drive"
        st.write(f"Status: {status_text}")

    with col_sync2:
        if st.button("⬆️ Labels nach Drive hochladen", key="ml_upload_drive"):
            df_local = st.session_state.get("df_plan_marie", None)
            if df_local is None:
                ...
                try:
                    ...
                    if "doc_id" not in df_remote.columns:
                        ...
                        st.success("Marie-Labels erfolgreich nach Google Drive hochgeladen (nur Marie-Spalten).")
                        st.rerun()
